...
To develop bitcore-node-artax:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-artax.git
...
We now will setup symlinks in `bitcore-node-artax` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-artax
ln -s ~/bitcoind-rpc-artax
```
...
cd devnode
mkdir node_modules
touch bitcore-node-artax.json
touch package.json
```

Edit `bitcore-node-artax.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-artax/bin/bitcore-node-artax start
```